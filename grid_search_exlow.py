...
def sah_standard(train_data, test_data):
    attr_list = ['nihtbx_fluidcomp_uncorrected','cbcl_scr_syn_internal_t.x','cbcl_scr_syn_external_t.x','smri_vol_cdk_paracnrh','smri_vol_cdk_parsopclh','asr_scr_totprob_r','smri_area_cdk_sutmlh','smri_vol_cdk_cuneuslh','smri_vol_cdk_precnlh','smri_area_cdk_locclh']
    scaler_list = dict()
    for i in attr_list:
        tmp = train_data[i]
        scaler = MinMaxScaler()
