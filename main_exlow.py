...
if __name__ == "__main__":
    ...
    for _ in range(n_runs):
        for train_index, val_index in skf.split(X, y):
            ...
            mesa.meta_fit(train_x_noFI, train_y_noFI, valid_x_noFI, valid_y_noFI, test_x_noFI, test_y_noFI)
            mesa.fit(train_x_noFI, train_y_noFI, valid_x_noFI, valid_y_noFI, verbose=False)
            
            mesa_auc_train_noFI = rater.score(train_y_noFI, mesa.predict_proba(train_x_noFI)[:, 1])
            y_pred_b_train_noFI = mesa.predict_proba(train_x_noFI)[:, 1]
            y_pred_b_train_noFI[y_pred_b_train_noFI < 0.5] = 0
            y_pred_b_train_noFI[y_pred_b_train_noFI >= 0.5] = 1
            mesa_recall_train_noFI = recall_score(train_y_noFI, y_pred_b_train_noFI, average=None)
            ...
            scores_list_train_noFI.append([svm_auc_train_noFI, tpr_svm_noFI, fpr_svm_noFI, ppv_svm_noFI,
                                      ...
                                      mesa_auc_train_noFI, mesa_recall_train_noFI[0], mesa_recall_train_noFI[1]])

            mesa_auc_valid_noFI = rater.score(valid_y_noFI, mesa.predict_proba(valid_x_noFI)[:, 1])
            y_pred_b_valid_noFI = mesa.predict_proba(valid_x_noFI)[:, 1]
            y_pred_b_valid_noFI[y_pred_b_valid_noFI < 0.5] = 0
            y_pred_b_valid_noFI[y_pred_b_valid_noFI >= 0.5] = 1
            mesa_recall_valid_noFI = recall_score(valid_y_noFI, y_pred_b_valid_noFI, average=None)
            ...
            scores_list_valid_noFI.append([svm_auc_valid_noFI, tpr_svm_noFI, fpr_svm_noFI, ppv_svm_noFI,
                                      ...
                                      mesa_auc_valid_noFI, mesa_recall_valid_noFI[0], mesa_recall_valid_noFI[1]])
...
# 设置列名
    merged_train_scores.columns = ['svm_auc',  'svm_sen','svm_spe', 'svm_ppv',
                                   'lr_auc',  'lr_sen','lr_spe', 'lr_ppv',
                                    ...
                                    'mesa_auc_noFI', 'mesa_spe_noFI', 'mesa_sen_noFI']
